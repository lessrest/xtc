...
- **Embedded Prolog (Trealla)**: Minimal static build of Trealla Prolog is
  linked; a Zig shim demonstrates basic evaluation (`src/trealla.zig`).
...
- No external C toolchain is required. Trealla's C sources are built via Zig's
  own C toolchain during `zig build`.
- macOS or Linux terminal

Optional (already vendored):

- `deps/trealla` is included as a submodule/vendor. If you cloned without
  submodules, run:
...
- Colors are resolved against a precomputed palette; not all Tailwind tokens are
  present.
...
- `src/trealla.zig` — Trealla Prolog shim
...
- `zg` (codeberg) for `Graphemes`, `DisplayWidth`, `Words`
- `pretty` (GitHub) — utilities used by tests/demo
- `deps/trealla` — C sources compiled as a static library (minimal features)
...
- Borders: styles (ASCII/Unicode mapping), colors, alpha backgrounds
...
- Portions of `src/xmlparse.zig` are derived from `zig-xml` by Meghan Denny
...
- `zig-xml` for the XML parser foundation.
- `zg` for grapheme, width, and word-iteration utilities.
- Trealla Prolog for the embedded engine.
