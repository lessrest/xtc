...
async function setup() {
  ...
  try {
    ...
    console.log("Make sure Caddy is running (the BSD setup should have started it)");
    process.exit(1);
  }
  ...
  if (!existsSync(resolve(PROJECT_DIR, certFile)) || !existsSync(resolve(PROJECT_DIR, keyFile))) {
    ...
    try {
      ...
      console.error("❌ Failed to generate certificate. Make sure mkcert is installed and CA is set up");
  ...
  try {
    ...
    const serversResponse = await fetch("http://localhost:2019/config/apps/http/servers");
    ...
    if (tlsResponse.ok) {
      ...
      if (existingTls?.certificates?.load_files) {
        ...
        if (!alreadyHasCert) {
          existingTls.certificates.load_files.push(tlsConfig.certificates.load_files[0]);
          tlsConfig.certificates.load_files = existingTls.certificates.load_files;
        } else {
    ...
    const tlsUpdateResponse = await fetch("http://localhost:2019/config/apps/tls", {
      method: "PUT",
      headers: {
        "Content-Type": "application/json"
      },
        ...
        for (const route of config.routes) {
          ...
          if (hosts.includes(DOMAIN)) {
            ...
            if (existsSync(certFile)) {
              ...
              console.log("⚠️  SSL certificate not found - run setup to generate");
            }
            break;
          }
        }
