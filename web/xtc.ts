...
class XTCModule {
    ...
    private allocateString(str: string): { ptr: number; length: number } {
        ...
        const ptr = (this.wasiInstance.instance.exports as any).wasm_alloc(bytes.length);
        if (!ptr) {
            throw new Error('Failed to allocate memory in WASM');
        }
        const memory = new Uint8Array((this.wasiInstance.instance.exports.memory as WebAssembly.Memory).buffer);
        ...
        try {
            ...
            if (this.wasiInstance.instance.exports.xtc_render) {
                ...
                if (xmlPtr) {
                    ...
                    this.wasiInstance.instance.exports.xtc_render(xmlPtr, xmlBytes.length, this.TERMINAL_COLS, this.TERMINAL_ROWS);
