...
//
// This entire file is derived from "zig-xml" by Meghan Denny (nektro)
// https://github.com/nektro/zig-xml (MPL-2.0). The code was merged into a
// single-file layout and lightly modified (e.g., tracer dependency stubbed)
// for ease of building in this project.
...
gentity_map: std.AutoArrayHashMapUnmanaged(StringIndex, StringIndex) = .{},
pentity_map: std.AutoArrayHashMapUnmanaged(StringIndex, StringIndex) = .{},
...
pub fn addElemNode(p: *Parser, alloc: std.mem.Allocator, ele: Element) !NodeIndex {
    const r = p.nodes.len;
    try p.nodes.append(alloc, .{ .element = ele });
    return @enumFromInt(r);
}

pub fn addTextNode(p: *Parser, alloc: std.mem.Allocator, txt: StringIndex) !NodeIndex {
    const r = p.nodes.len;
    try p.nodes.append(alloc, .{ .text = txt });
    return @enumFromInt(r);
}

pub fn addPINode(p: *Parser, alloc: std.mem.Allocator, pi: ProcessingInstruction) !NodeIndex {
    const r = p.nodes.len;
...
pub fn addNodeList(p: *Parser, alloc: std.mem.Allocator, items: []const NodeIndex) !NodeListIndex {
    if (items.len == 0) return .empty;
    const r = p.data.items.len;
    try p.data.ensureUnusedCapacity(alloc, 1 + items.len);
    p.data.appendAssumeCapacity(@intCast(items.len));
...
fn collectAttributes(alloc: std.mem.Allocator, p: *Parser) !AttributeListIndex {
    var list = std.ArrayList(StringIndex){} ;
...
fn parseContent(alloc: std.mem.Allocator, p: *Parser) anyerror!?NodeListIndex {
    ...
    var list1 = std.ArrayList(NodeIndex){} ;
    defer list1.deinit(alloc);
    var list2 = std.ArrayList(u8){} ;
    defer list2.deinit(alloc);

    try addOpStringToList(p, alloc, &list2, try parseCharData(alloc, p));
    while (true) {
        ...
        if (try parseReference(alloc, p)) |ref| {
            try addReferenceToList(p, alloc, &list2, ref);
            try addOpStringToList(p, alloc, &list2, try parseCharData(alloc, p));
            continue;
        }
        if (try parseCDSect(alloc, p)) |cdata| {
            try addOpStringToList(p, alloc, &list2, cdata);
            try addOpStringToList(p, alloc, &list2, try parseCharData(alloc, p));
            continue;
        }
...
fn parseAttValue(alloc: std.mem.Allocator, p: *Parser) anyerror!?StringIndex {
    ...
    while (true) blk: {
        ...
        if (try parseReference(alloc, p)) |ref| break :blk try addReferenceToList(p, alloc, &list, ref);
        unreachable;
    }
    return try p.addStr(alloc, list.items);
}
...
fn addReferenceToList(p: *Parser, alloc: std.mem.Allocator, list: *std.ArrayList(u8), ref: Reference) !void {
    return switch (ref) {
...
fn addOpStringToList(p: *Parser, alloc: std.mem.Allocator, list: *std.ArrayList(u8), sidx_maybe: ?StringIndex) !void {
    // try list.appendSlice(alloc, p.getStr(sidx_maybe orelse try p.addStr(list.allocator, "")));
