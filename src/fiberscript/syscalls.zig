...
fn buildTestSyscallsImpl(comptime SyscallsType: type) type {
    return struct {
        pub fn print(context: *TestContext, payload: SyscallsType.Payload(.print)) []const u8 {
            context.output_buffer.appendSlice(payload.message) catch unreachable;
            return payload.message;
        }
        
        pub fn readFile(context: *TestContext, payload: SyscallsType.Payload(.readFile)) []const u8 {
            _ = context;
        ...
        pub fn sleep(context: *TestContext, payload: SyscallsType.Payload(.sleep)) void {
            _ = context;
    ...
    inline for (std.meta.fields(Interface)) |field| {
        ...
        if (expected_type != actual_type) {
            @compileError(std.fmt.comptimePrint(
                "Function signature mismatch for {s}: expected {}, got {}", 
...
pub fn RequestUnion(comptime Syscalls: type) type {
    ...
    union_fields[0] = .{
        .name = "Ring.push",
        .type = struct { ring: *anyopaque },
        .alignment = @alignOf(struct { ring: *anyopaque }),
    };
    ...
    return struct {
        pub fn parseRequest(slot_map: anytype) !Request {
            ...
            if (std.mem.eql(u8, operation, "Ring.push")) {
                const ring = try slot_map.lookup("ring", *anyopaque);
                ...
                inline else => |payload, tag| {
                    ...
                    inline for (syscall_fields) |field| {
                        if (comptime std.mem.eql(u8, @tagName(tag), field.name)) {
                            ...
                            // TODO: Handle return value - put in completion queue
                            _ = result;
                            return;
                        }
                    }
