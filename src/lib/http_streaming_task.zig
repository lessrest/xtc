...
pub fn HttpStreamingTask(comptime Fiber: type) type {
    ...
    return struct {
        ...
        pub fn getSupervisionStatus(self: *Self, buf: []u8) []const u8 {
            if (self.pom) |_| {
                return std.fmt.bufPrint(buf, "HTTP tasks under scope {d}, total requests: {d}", .{ self.http_scope, self.base.requests.count() }) catch "status unavailable";
            }
            return std.fmt.bufPrint(buf, "No supervision (legacy mode), requests: {d}", .{self.base.requests.count()}) catch "status unavailable";
        }
