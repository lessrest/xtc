...
class ClockTetris {
  ...
  start() {
    spawnPiece()
    updateBoard()
    // Clocks start automatically when added to DOM
  }
  
  pause() {
    // Update clock styles to pause them
    _dropClock.updateClass("clock interval-500 clock-hidden")
    _speedClock.updateClass("clock interval-10000 clock-hidden")
  }
  
  resume() {
    // Resume clocks by showing them again
    _dropClock.updateClass("clock interval-500 clock-spinner bg-blue-500 text-white p-1")
    _speedClock.updateClass("clock interval-10000 clock-pulse bg-yellow-500 text-white p-1")
  }
  ...
  increaseLevel() {
    ...
    _level = _level + 1
    _levelText.updateText("Level: " + _level.toString)
    
    // Speed up the drop clock
    var newInterval = (500 - (_level - 1) * 50).max(100)
    _dropClock.updateClass("clock interval-" + newInterval.toString + " clock-spinner bg-blue-500 text-white p-1")
  }
  ...
  spawnPiece() {
    ...
    if (!canPlace(_currentShape, _currentX, _currentY)) {
      ...
      _gameOverText.updateText("GAME OVER! Score: " + _score.toString + " Press R to restart")
    }
  }
  ...
  clearLines() {
    ...
    if (linesCleared > 0) {
      // Score more for multiple lines
      var points = linesCleared * 100 * _level
      if (linesCleared == 4) points = points * 2  // Tetris bonus
